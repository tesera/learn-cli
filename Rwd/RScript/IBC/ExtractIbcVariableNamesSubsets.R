...
arrayDim <- dim(ibcVariableSets$subsets)
...
for (k in 1:maxSolutionTypes){
	for (i in 1: maxSolutions) {
		...
		for (j in 1:maxVariables){
			if (ibcVariableSets$subsets[i,j,k] > 0) {
				uid <- uid + 1
				UID <- c(UID, uid)
				MODELID <- c(MODELID, mindex)
				SOLTYPE <- c(SOLTYPE,k)
				SOLNUM <- c(SOLNUM,i)
				VARNUM <- c(VARNUM,ibcVariableSets$subsets[i,j,k])
				VARNAME <- c(VARNAME,xNames[ibcVariableSets$subsets[i,j,k]])
				KVAR <- c(KVAR,j)
			}	
		}
	}
