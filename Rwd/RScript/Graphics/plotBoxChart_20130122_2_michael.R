...
#myplot <- ggplot (mydata, aes(Species, Sepal.Length, colour=Species)) + geom_boxplot() + opts(title="My DATA EXAMPLE") 
...
myplot <- ggplot (mydata, aes(Species, Sepal.Length, colour=Species)) + geom_boxplot() + scale_colour_manual(breaks=c("setosa","versicolor","virginica"),values=c("red","blue","#F4F5EB")) + opts(title="My DATA EXAMPLE") 
myplot 
